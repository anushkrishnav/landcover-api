	...
	if current_user.is_authenticated:
		...
		for person in user.followers:
			if user.is_following(person):
				mutual += 1
		return render_template('profile.html', user=user, users=users, mutual=mutual, title=user.username.title(), dp=dp, form=form)
	user = User.query.filter_by(username=username).first_or_404()
	return render_template('profile.html', user=user, title=user.username.title())
